...
class Fista(unittest.TestCase):
	...
	def test_project_on_p_output_type(self):
		out_int32 = fista.project_on_P( self.p_rand_int32_matrix_light)
		self.assertEqual(np.float64,out_int32.dtype)

		out_uint32 = fista.project_on_P( self.p_rand_uint32_matrix_light)
		self.assertEqual(np.float64,out_uint32.dtype)

		out_float32 = fista.project_on_P( self.p_rand_float32_matrix_light)
		self.assertEqual(np.float64,out_float32.dtype)		

	def test_project_on_p_correctness(self):

		out_static_float32 = fista.project_on_P( self.p_static_float32_matrix )
